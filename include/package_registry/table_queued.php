<?php
$limit = 10;  
if (isset($_GET["pageQueued"])) { $pageQueued  = $_GET["pageQueued"]; } else { $pageQueued=1; };  
$start_from = ($pageQueued-1) * $limit;  
include ('php/establish_link.php');
//$q = "SELECT * FROM `Package_List` WHERE `WiD`='$WiD' AND `Status`='Queued' ASC LIMIT ('$start_from, $limit')";
$q = "SELECT * FROM `Package_List` WHERE `WiD`='$WiD' AND `Status`='Queued' ORDER BY `Package_Number` ASC LIMIT $start_from, $limit";
...
?>
<div class="row" id="queded">
			<div class="row">
<div class="col-lg-12">
                    <div class="panel panel-default">
                        <div class="panel-heading">
                            Packages In Quede (No Email Sent to Resident)
                        </div>
                        <!-- /.panel-heading -->
...
<?php  
$sql = "SELECT COUNT(`WiD`) FROM Package_List WHERE `WiD`='$WiD' AND Status='Queued'";  
...
for ($i=1; $i<=$total_pages; $i++) {  
             $pagLink .= "<li class='paginate_button' aria-controls='dataTables-example' tabindex='0'><a href='resident_profile.php?WiD=$WiD#Queded&pageQueued=".$i."'>".$i."</a></li>";  
};  

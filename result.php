...
if($_POST)
{
    ...
    $strSQL_Result = mysqli_query($connection,"select id,name,WiD from live_search where name like '%$q%' or WiD like '%$q%' order by id LIMIT 5");
    while($row=mysqli_fetch_array($strSQL_Result))
    {
        $username   = $row['name'];
        $WiD     = $row['WiD'];
        $b_username = '<strong>'.$q.'</strong>';
        $b_WiD    = '<strong>'.$q.'</strong>';
        $final_username = str_ireplace($q, $b_username, $username);
        $final_email = str_ireplace($q, $b_WiD, $WiD);
        ?>
